...
class Pet {
  ...
  buscaPorId(id) {
    const sql = `SELECT FROM Pets.id, Pets.nome, Pets.tipo, Pets.observacoes, Clientes.id as donoId,
                 Clientes.nome as donoNome, Clientes.cpf as donoCpf INNER JOIN 
                 Clientes WHERE Pets.id=${parseInt(id)} AND Clientes.id = Pets.donoId`
